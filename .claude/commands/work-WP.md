...
- `/work-wp IDEA-0002-my-idea` → run the next `Queued` WP for that idea
- `/work-wp IDEA-0002-my-idea WP-0002` → run a specific WP for that idea
- `/work-wp WP-0002` → best-effort: locate the WP by scanning all ideas (only if unique)
...
## Step 1 — Resolve IDEA_ID and target WP
...
- If zero matches: STOP and report “WP not found in any idea.”
...
## Step 1.1 — Confirm idea-scoped files exist
...
## Step 2 — Select the Work Package (WP)
...
- Report “No queued WPs found for <IDEA_ID>.”
...
## Step 3 — Auto-update WP status to In Progress
...
## Step 6 — Execute One Task at a Time (Iterative Loop)
...
### 6.4 — Update WP Plan Doc checkbox
...
### 6.5 — Update manifest (task status)
...
## Step 7 — WP-level Verification + Close Out
...
## Step 9 — End-of-Run Summary (Always)
