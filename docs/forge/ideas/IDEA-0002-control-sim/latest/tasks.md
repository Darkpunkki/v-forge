...
- None
  ...
  - id: "TASK-060"
    ...
    description: "Extend Session model in apps/api/vibeforge_api/core/session.py to include LLM mode toggle, provider selection, model name, temperature, cost tracking, and history depth fields. Update serialization and API responses. Session defaults to stub mode (use_real_llm=False) for safety."
    acceptance_criteria:
      - "Session model includes: use_real_llm (bool, default False), llm_provider (str, default 'openai'), default_model (str, default 'gpt-4o-mini'), default_temperature (float, default 0.7), simulation_cost_usd (float, default 0.0), max_history_depth (int, default 20)."
  ...
  - id: "TASK-061"
    ...
    acceptance_criteria:
      ...
      - "POST /control/sessions/{id}/simulation/config accepts max_cost_usd and tick_rate_limit_ms."
  ...
  - id: "TASK-062"
    ...
    description: "Create orchestration/coordinator/llm_response_generator.py with LlmResponseGenerator class. Implement role-based system prompts (orchestrator, worker, reviewer, fixer, foreman) and conversation history building. Extend the existing orchestration/prompts.py (or add a new prompts module) to include agent role prompts. This service handles all LLM interactions for agent responses."
    acceptance_criteria:
      ...
      - "System prompts defined for all 5 roles in orchestration/prompts.py (or a dedicated prompts module) and imported by LlmResponseGenerator."
  ...
  - id: "TASK-065"
    ...
    description: "Extend SimulationConfig.tsx (used by /simulation) to include toggle for stub vs real LLM mode, model selector, cost budget input, and rate limit input. Display current cost in TickControls.tsx. Provide clear warnings about API costs to prevent accidental spend."
    acceptance_criteria:
      ...
      - "Toggle and inputs disabled once simulation is started (based on tick_status)."
