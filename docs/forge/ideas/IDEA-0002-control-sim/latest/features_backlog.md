...
features:
  ...
  - id: "FEAT-002"
    ...
    description: "Defines directed or bidirectional links between agents (including cycles) and validates only that endpoints exist. Surfaces validation failures with clear reasons."
    acceptance_criteria:
      ...
      - "Bidirectional links and cycles are accepted when agent ids are valid."
      - "Stored links are retrievable as the current communication graph."
    in_scope:
      - "Directed and bidirectional link definitions, including cycles."
    out_of_scope:
      - "Graph validation beyond edge existence (for example, DAG enforcement)."
    dependencies: []
    release_target: "MVP"
    priority: "P0"
    tags: ["config", "simulation", "api"]
  ...
  - id: "FEAT-004"
    ...
    description: "Advancing a tick increments the tick index, processes exactly one queued event from a FIFO queue, and produces per-tick results."
    acceptance_criteria:
      - "When a tick is advanced, the tick index increases by one and is reflected in session state."
      - "Each tick processes exactly one queued event (send or response) from the FIFO queue."
      - "Given multiple queued events, the first queued event is processed and remaining events stay queued."
  ...
  - id: "FEAT-005"
    ...
    acceptance_criteria:
      ...
      - "Blocked sends create a message log entry like \"Blocked: A -> B not allowed\" as a system event."
  ...
  - id: "FEAT-008"
    ...
    acceptance_criteria:
      ...
      - "Reset clears message log/state, sets tick=0, and preserves configuration when requested."
  ...
  - id: "FEAT-013"
    ...
    acceptance_criteria:
      ...
      - "System entries for blocked sends display \"Blocked: A -> B not allowed\"."
