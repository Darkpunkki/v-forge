...
class WorkspaceManager:
    ...
    def init_repo(
        ...
        Args:
            ...
            enable_git: Optional flag to initialize git repository
    ...
    def commit_snapshot(self, session_id: str, message: str, allow_empty: bool = False) -> bool:
        """Commit a git snapshot for the workspace repo.

        Args:
            session_id: Session identifier
            message: Commit message for the snapshot
            allow_empty: Allow empty commits when no changes are present

        Returns:
            True if a commit was created, False if skipped due to no changes
        """
        repo_path = self.get_repo_path(session_id)
        self._ensure_git_repo(repo_path)

        status = self._run_git(repo_path, ["status", "--porcelain"]).stdout.strip()
        ...
        self._run_git(repo_path, commit_command)
        return True
    ...
    def _ensure_git_available(self):
        if shutil.which("git") is None:
            raise ValueError("Git is not available in the current environment.")

    def _ensure_git_repo(self, repo_path: Path):
        if not (repo_path / ".git").exists():
            raise ValueError(f"Git repository not initialized at {repo_path}")
